...
import emu_like.io as io  # noqa: E402
...
for name, mask in regions.items():
    ...
    print("    mean(ratio) range: "
          f"[{mean_spec.min():.4f}, {mean_spec.max():.4f}]")
...
for i, idx in enumerate(idx_sample):
    ...
    for ip in range(8):
        ...
        if mask_good.any():
            ...
            sensitivities[i, ip] = np.mean(
                dy_norm[mask_good] / abs_dx[mask_good])
...
for i in range(len(m_bins)-1):
    ...
    axs[row, col].set_title(
        f'm_ncdm in [{m_bins[i]:.2f}, {m_bins[i+1]:.2f}] (N={n})')
...
for i, m_range in enumerate([
        (0, 0.03), (0.03, 0.06), (0.06, 0.09), (0.09, 0.12)]):
    mask_lo = (m_ncdm >= m_range[0]) & (m_ncdm < m_range[1]) & N_ur_lo
    mask_hi = (m_ncdm >= m_range[0]) & (m_ncdm < m_range[1]) & N_ur_hi
    if mask_lo.sum() > 0:
        axs[1, i].plot(
            np.mean(y[mask_lo], axis=0),
            label=f'N_ur<0.2 (N={mask_lo.sum()})',
            color='blue')
    if mask_hi.sum() > 0:
        axs[1, i].plot(
            np.mean(y[mask_hi], axis=0),
            label=f'N_ur>0.8 (N={mask_hi.sum()})', color='red')
...
# Row 2: Ratio of the low/high N_ur mean spectra per ell
# (how much N_ur changes the shape)
for i, m_range in enumerate([
        (0, 0.03), (0.03, 0.06), (0.06, 0.09), (0.09, 0.12)]):
    mask_lo = (m_ncdm >= m_range[0]) & (m_ncdm < m_range[1]) & N_ur_lo
    mask_hi = (m_ncdm >= m_range[0]) & (m_ncdm < m_range[1]) & N_ur_hi
...
plt.tight_layout()
fig.savefig(os.path.join(SAVE_DIR, 'diag_regime_spectra.png'),
...
print(f"N_eff_approx range: "
      f"[{N_eff_approx.min():.4f}, {N_eff_approx.max():.4f}]")
...
for ip in range(8):
    ...
    print(f"  {PARAM_NAMES[ip]:>15s}: r(mean)={r1:+.4f}, "
          f"r(log_mean)={r3:+.4f}")
...
sc = axs[0, 0].scatter(
    N_eff_approx, y_mean_per_sample, s=0.3, alpha=0.2,
    c=m_ncdm, cmap='viridis')
...
sc = axs[0, 1].scatter(
    f_nu, y_mean_per_sample, s=0.3, alpha=0.2, c=N_ur, cmap='viridis')
...
sc = axs[0, 2].scatter(
    N_ur, m_ncdm, c=y_mean_per_sample, s=0.3, alpha=0.3, cmap='RdYlBu_r')
...
sc = axs[1, 0].scatter(
    N_ur, m_ncdm, c=np.log10(y_std_per_sample),
...
sc = axs[1, 1].scatter(
    N_ur, m_ncdm, c=log_y_var, s=0.3, alpha=0.3, cmap='RdYlBu_r')
...
sc = axs[1, 2].scatter(
    N_ur, m_ncdm, c=np.log10(shape_dist),
    s=0.3, alpha=0.3, cmap='RdYlBu_r')
...
plt.tight_layout()
fig.savefig(os.path.join(SAVE_DIR, 'diag_regime_physics.png'),
            dpi=150, bbox_inches='tight')
plt.close(fig)
...
for tag in ['thin', 'std', 'ext']:
    ...
    # Compute the "noise" level: how much do nearby parameter points
    # differ in spectra?
    ...
    print(f"\n{tag}:")
    print(f"  Parameter range (N_ur): "
          f"[{x_t[:, 6].min():.4f}, {x_t[:, 6].max():.4f}]")
    print(f"  Parameter range (m_ncdm): "
          f"[{x_t[:, 7].min():.6f}, {x_t[:, 7].max():.6f}]")
    print(f"  NN relative diff: median={np.median(nn_diffs):.4e}, "
          f"P90={np.percentile(nn_diffs, 90):.4e}, "
          f"P99={np.percentile(nn_diffs, 99):.4e}")
    print(f"  Spectrum range: [{y_t.min():.4e}, {y_t.max():.4e}]")
    print(f"  Log variance per feature: "
          f"median={np.median(np.var(np.log(y_t), axis=0)):.4e}")
    print(f"  Max/Min of mean spectrum: "
          f"{np.max(np.mean(y_t, axis=0))/np.min(np.mean(y_t, axis=0)):.4f}")
