...
if __name__ == '__main__':
    ...
    str_out_1 = '\x1b[1;32m[info]\x1b[00m Resuming from '
    str_out_2 = '\x1b[1;32m----> \x1b[00mWriting output in '
    ...
    for log_file in args.log_files:
        ...
        for line in head:
            if line.startswith(str_out_1):
                output_file = line.replace(str_out_1, '')[:-1]
            elif line.startswith(str_out_2):
                output_file = line.replace(str_out_2, '')
        ...
        samples_this_run, remaining_this_run = [
            int(x) for x in tail.split('/')]
        ...
        io.print_level(1, 'Remaining samples to run: {}'.format(
            samples_tot-samples_run))
        io.print_level(1, 'Remaining samples to save: {}'.format(
            samples_tot-samples_saved))
        io.print_level(1, 'Remaining samples to next save: {}'.format(
            save_interval-samples_run+samples_saved))
        print()
